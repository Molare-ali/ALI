...
export default function CartPage() {
  ...
  return (
    <section className="luxury-container py-12">
      <p className="fine-label text-champagne">Shopping Cart</p>
      <h1 className="serif-title mb-8 text-5xl text-aubergine">Your Selection</h1>
      {!items.length ? (
        <div className="border border-champagne/30 bg-ivory p-10">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_340px]">
          <motion.div layout className="grid gap-4">
            ...
            {items.map((item, index) => (
              <motion.div
                ...
                className="grid gap-4 border border-champagne/30 bg-ivory p-4 md:grid-cols-[110px_1fr_auto]"
              >
                ...
                <div>
                  ...
                  <p className="mt-2 flex flex-wrap items-center gap-2 text-sm text-onyx/62">
                    ...
                    {item.colorHex && <span className="inline-block h-4 w-4 border border-champagne/50" style={{ backgroundColor: item.colorHex }} />}
                    {item.color}
                  </p>
                  <div className="mt-4 flex flex-wrap items-center gap-3">
                    ...
                    <button onClick={() => removeItem(index)} className="inline-flex h-11 items-center gap-2 border border-champagne/40 px-3 text-sm text-aubergine">
          ...
          <aside className="h-fit border border-champagne/30 bg-aubergine p-6 text-ivory">
            <p className="fine-label text-champagne">Total Price</p>
            <p className="serif-title mt-3 text-4xl">{formatCurrency(total)}</p>
            {!user && <p className="mt-5 border border-champagne/30 p-4 text-sm leading-6 text-ivory/82">Please log in or create an account to complete your order.</p>}
            <Button href={user ? "/checkout" : "/login?redirect=/checkout"} className="mt-6 w-full bg-champagne text-aubergine hover:bg-ivory">
