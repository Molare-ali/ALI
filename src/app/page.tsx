    ...
    <>
      <section className="relative overflow-hidden bg-aubergine text-ivory">
        ...
        <div className="luxury-container relative grid min-h-[calc(100vh-80px)] content-center gap-8 py-20">
          <Reveal className="max-w-3xl">
            <p className="fine-label mb-5 text-champagne">Luxury Italian Sartorial Fashion</p>
            ...
            <div className="mt-9 flex flex-wrap gap-4">
              ...
              <Button href="/contact" variant="secondary" className="border-champagne text-ivory hover:bg-champagne/20">Contact Us</Button>
            </div>
          </Reveal>
          <Reveal delay={0.12} className="mt-10 grid max-w-4xl grid-cols-1 border-y border-champagne/35 py-5 sm:grid-cols-3">
            {["Aubergine tailoring", "Champagne details", "Warm ivory restraint"].map((item) => (
      ...
      <Reveal className="luxury-container py-20">
        <div className="mb-9 flex flex-wrap items-end justify-between gap-5">
          <div>
            <p className="fine-label text-champagne">Featured Products</p>
      ...
      <section className="bg-plum py-18 text-ivory">
        <Reveal className="luxury-container grid gap-6 py-16 md:grid-cols-4">
          {categories.map((category) => (
            <Link key={category.id} href={`/shop?category=${category.slug}`} className="group border border-champagne/30 p-6 transition hover:bg-ivory hover:text-aubergine">
              <p className="fine-label text-champagne">Category</p>
      ...
      <Reveal className="luxury-container grid gap-10 py-20 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        ...
        <div className="grid gap-6">
          <p className="fine-label text-champagne">Brand Story</p>
