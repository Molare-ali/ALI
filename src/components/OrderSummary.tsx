...
export function OrderSummary({ items, order }: { items?: CartItem[]; order?: Order }) {
  ...
  return (
    <section className="border border-champagne/30 bg-ivory p-6 shadow-luxury">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          <p className="fine-label text-champagne">Invoice</p>
          <h2 className="serif-title text-3xl text-aubergine">{order?.orderNumber || "Order Summary"}</h2>
        </div>
        {order && <span className="border border-champagne/45 px-3 py-2 text-sm text-aubergine">{order.status}</span>}
      </div>
      <div className="grid gap-4">
        {orderItems.map((item, index) => (
          <div key={`${item.productId}-${item.size}-${item.color}-${index}`} className="grid grid-cols-[64px_1fr_auto] gap-4 border-b border-champagne/20 pb-4">
            ...
            <div>
              ...
              <p className="mt-1 flex flex-wrap items-center gap-2 text-sm text-onyx/60">
                Size {item.size} / {item.colorHex && <span className="inline-block h-4 w-4 border border-champagne/50" style={{ backgroundColor: item.colorHex }} />} {item.color} / Qty {item.quantity}
              </p>
            </div>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-champagne/40 pt-5">
